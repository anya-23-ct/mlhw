...
class SymmetricQuantFunction(torch.autograd.Function):
    ...
    def forward(ctx, x, k, specified_scale=None, specified_zero_point=None):
        ...
        # TODO: fill-in (start)
        # raise NotImplementedError
        # output = torch.clamp(ste_round().forward(ctx, x / scale) + zero_point, -2**(k-1), 2**(k-1) - 1)
...
class QConfig(object):
    ...
    def get_quantization_params(self, saturation_min, saturation_max):
        ...
        with torch.no_grad():
            if self.is_symmetric:
                ...
                scale = max(abs(saturation_max),abs(saturation_min))/ (2 ** (self.quant_bits - 1) - 1)
    ...
    def quantize_with_min_max(self, x, saturation_min, saturation_max, fake_quantize=False):
        ...
        # print('saturation_min {} saturation_max {} scale {} zero_point {} fakequant {}'.format(saturation_min,saturation_max,scale,zero_point,fake_quantize))
...
def quantize_weights_bias(w, qconfig, fake_quantize=False):
    ...
    # TODO: fill-in (start)
    # raise NotImplementedError
    w_detach = w.data.detach()
    w_min = torch.min(w_detach)
    # print('w_min is {}'.format(w_min))
    w_max = torch.max(w_detach)
    # print('w_max is {}'.format(w_max))
    w_q = qconfig.quantize_with_min_max(w, w_min, w_max, fake_quantize=fake_quantize)
    # print('w_q is {}'.format(w_q))
    # TODO: fill-in (end)
